...
import { Client, GatewayIntentBits, PermissionFlagsBits as PermissionFlagsBits2, MessageFlags, ButtonBuilder, ButtonStyle, ActionRowBuilder, AttachmentBuilder, TextInputBuilder, TextInputStyle, ModalBuilder } from "discord.js";
...
Rules: messages or qualifying Discord voice time; LOA, excluded roles and new members excluded. Game presence alone is never grounds for a flag.`, [new AttachmentBuilder(Buffer.from(csv([["member_id"], ...result.ids.map((id) => [id])])), { name: "inactive.csv" })]);
  ...
  if (sub === "role") {
    ...
    return respond(i, `Current members of ${role.name} (${ids2.length}, including zero activity)${channel ? ` in channel ${channel.id}` : ""}:
...
Existing: ${old.name}; ${stamp(old.start_at)}\u2013${stamp(old.end_at)}; channels ${JSON.parse(old.channels).join(", ")}; minimum ${old.min_ms / 6e4}m
New values: ${JSON.stringify(patch)}`, () => {
...
${report.recalculated ? "Recalculated after event settings changed. " : ""}${report.coverage.incomplete ? "Coverage incomplete from tracking start, retention, or gaps. " : ""}${gaps ? `Known unobserved windows: ${gaps}. ` : ""}${report.event.source_url ?? ""}`;
    const files = i.options.getBoolean("csv") ? [new AttachmentBuilder(Buffer.from(csv([["member_id", "arrival_utc", "departure_utc", "measured_ms", "manual_minutes", "met_minimum"], ...rows.map((x) => [x.user, x.arrival ? new Date(x.arrival).toISOString() : "", x.departure ? new Date(x.departure).toISOString() : "", x.measuredMs, x.adjustmentMinutes, x.metMinimum])])), { name: `attendance-${id}.csv` })] : void 0;
...
async function privacyCommand(i, g) {
  ...
  if (sub === "delete-member") {
    ...
    return preview(i, `Permanently delete stored activity, attendance corrections, and member metadata for ${u.id} in this server?`, () => {
      store.deleteMember(g, u.id);
      return `Deleted stored activity for ${u.id}.`;
    });
  }
  return preview(i, `Permanently reset all activity, events, gaps, and corrections in server ${g}?`, () => {

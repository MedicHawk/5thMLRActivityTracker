...
export const defaults = { timezone: 'America/New_York', excludedChannels: [], excludedCategories: [], excludedRoles: [], loaRoles: [], excludeSelfDeaf: false, requireTwoHumans: false, participationCap: 5, aliases: { 'arma reforger': 'Arma Reforger' }, apolloBotId: '', retentionDays: 365, newcomerGraceDays: 7 };
export class Store {
    db;
    constructor(path) {
        if (path !== ':memory:')
    ...
    migrate() {
        ...
        if (this.db.prepare('SELECT 1 FROM migrations WHERE version=1').get())
            return;
    ...
    ensureGuild(guild, now) { this.db.prepare('INSERT OR IGNORE INTO guilds VALUES(?,?,?)').run(guild, now, JSON.stringify(defaults)); this.db.prepare('INSERT OR IGNORE INTO heartbeats VALUES(?,?)').run(guild, now); }
    config(guild) { const row = this.db.prepare('SELECT config FROM guilds WHERE guild_id=?').get(guild); return { ...defaults, ...(row ? JSON.parse(row.config) : {}) }; }
    setConfig(guild, value) { this.db.prepare('UPDATE guilds SET config=? WHERE guild_id=?').run(JSON.stringify(value), guild); }
    member(guild, user, joined) { this.db.prepare('INSERT INTO members VALUES(?,?,?) ON CONFLICT(guild_id,user_id) DO UPDATE SET joined_at=COALESCE(excluded.joined_at,members.joined_at)').run(guild, user, joined); }
    ...
    live(guild, user, kind) { return this.db.prepare('SELECT * FROM live WHERE guild_id=? AND user_id=? AND kind=?').get(guild, user, kind); }
    startLive(guild, user, kind, at, qualifies, channel, game) { this.db.prepare('INSERT INTO live VALUES(?,?,?,?,?,?,?,?) ON CONFLICT(guild_id,user_id,kind) DO UPDATE SET channel_id=excluded.channel_id,game=excluded.game,since_at=excluded.since_at,checkpoint_at=excluded.checkpoint_at,qualifies=excluded.qualifies').run(guild, user, kind, channel ?? null, game ?? null, at, at, qualifies ? 1 : 0); }
    flushLive(guild, user, kind, at, remove = false) {
        ...
        if (remove)
            this.db.prepare('DELETE FROM live WHERE guild_id=? AND user_id=? AND kind=?').run(guild, user, kind);
        else
            this.db.prepare('UPDATE live SET since_at=?,checkpoint_at=? WHERE guild_id=? AND user_id=? AND kind=?').run(end, end, guild, user, kind);
    }
    checkpoint(at) { this.db.transaction(() => { const rows = this.db.prepare('SELECT * FROM live').all(); for (const r of rows)
        this.flushLive(r.guild_id, r.user_id, r.kind, at); this.db.prepare('UPDATE heartbeats SET last_at=?').run(at); })(); }
    recover(at) {
        this.db.transaction(() => {
            const rows = this.db.prepare('SELECT * FROM live').all();
            for (const r of rows)
                this.flushLive(r.guild_id, r.user_id, r.kind, r.checkpoint_at, true);
            for (const hb of this.db.prepare('SELECT guild_id,last_at FROM heartbeats').all())
                if (at > hb.last_at)
                    this.gap(hb.guild_id, hb.last_at, at, 'Bot disconnected or restarted');
            this.db.prepare('UPDATE heartbeats SET last_at=?').run(at);
        })();
    }
    gap(guild, start, end, reason) { if (end > start)
        this.db.prepare('INSERT INTO gaps(guild_id,start_at,end_at,reason) VALUES(?,?,?,?)').run(guild, start, end, reason); }
    prune(guild, now) { const cutoff = now - this.config(guild).retentionDays * 86_400_000; this.db.transaction(() => { this.db.prepare('DELETE FROM messages WHERE guild_id=? AND at<?').run(guild, cutoff); this.db.prepare('DELETE FROM voice_sessions WHERE guild_id=? AND at<?').run(guild, cutoff); this.db.prepare('DELETE FROM intervals WHERE guild_id=? AND end_at<?').run(guild, cutoff); this.db.prepare('UPDATE intervals SET start_at=? WHERE guild_id=? AND start_at<? AND end_at>?').run(cutoff, guild, cutoff, cutoff); })(); }
    deleteMember(guild, user) { this.db.transaction(() => { for (const table of ['messages', 'intervals', 'live', 'voice_sessions', 'corrections', 'members'])
        this.db.prepare(`DELETE FROM ${table} WHERE guild_id=? AND user_id=?`).run(guild, user); })(); }
    resetGuild(guild, now) { this.db.transaction(() => { for (const table of ['messages', 'intervals', 'live', 'voice_sessions', 'gaps', 'events', 'event_audit', 'corrections', 'members'])
        this.db.prepare(`DELETE FROM ${table} WHERE guild_id=?`).run(guild); this.db.prepare('UPDATE guilds SET started_at=? WHERE guild_id=?').run(now, guild); this.db.prepare('UPDATE heartbeats SET last_at=? WHERE guild_id=?').run(now, guild); })(); }
    close() { this.db.close(); }
}
